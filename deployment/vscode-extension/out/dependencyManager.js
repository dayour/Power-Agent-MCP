...
class DependencyManager {
     ...
     */
    async installMissingDependencies(showProgress = true) {
        ...
        const result = await vscode.window.showInformationMessage(`The following dependencies are missing: ${depNames}. Would you like to install them automatically?`, 'Install All', 'Install Individually', 'Skip');
     ...
     */
    async installDependenciesWithProgress(dependencies) {
     ...
     */
    async installDependenciesIndividually(dependencies) {
        for (const dep of dependencies) {
            const result = await vscode.window.showInformationMessage(`Install ${dep.displayName}?`, 'Yes', 'No', 'Manual Instructions');
            if (result === 'Yes') {
                try {
                    if (dep.installer) {
                        await vscode.window.withProgress({
     ...
     */
    async showManualInstallInstructions(dep) {
        ...
        const result = await vscode.window.showInformationMessage(instructions, 'Open Download Page', 'Copy Command', 'OK');
     ...
     */
    async getMissingDependenciesCount() {
        const dependencies = await this.checkDependencies();
