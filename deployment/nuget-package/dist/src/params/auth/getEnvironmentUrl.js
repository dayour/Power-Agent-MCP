...
export function getEnvironmentUrl() {
    ...
    if (!endpointUrl) {
        const envParams = GetPipelineOutputVariable(variableName);
        endpointUrl = envParams.value;
        const taskName = envParams.taskName;
        if (endpointUrl) {
            if (taskName) {
...
export function readEnvUrlFromServiceConnection(defaultAuthType) {
