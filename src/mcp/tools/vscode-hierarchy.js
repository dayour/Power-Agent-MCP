...
exports.VSCodeHierarchicalTools = void 0;
/**
 ...
 */
var VSCodeHierarchicalTools = /** @class */ (function () {
    function VSCodeHierarchicalTools(fullHandler) {
        this.fullHandler = fullHandler;
    }
    VSCodeHierarchicalTools.prototype.getParentTools = function () {
        return [
            {
                name: 'pp_environment',
    ...
    VSCodeHierarchicalTools.prototype.getHandlers = function () {
        var _this = this;
        var handlers = {};
        // Create dispatch handlers for each parent tool
        var parentTools = this.getParentTools();
        parentTools.forEach(function (parentTool) {
            handlers[parentTool.name] = function (args) { return __awaiter(_this, void 0, void 0, function () {
                var command, _a, parameters, actualToolName;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            command = args.command, _a = args.parameters, parameters = _a === void 0 ? {} : _a;
                            if (!command) {
                                throw new Error("Command parameter is required for ".concat(parentTool.name));
                            }
                            actualToolName = this.mapCommandToToolName(command);
                            return [4 /*yield*/, this.fullHandler.callTool(actualToolName, parameters)];
                        case 1: 
    ...
    VSCodeHierarchicalTools.prototype.mapCommandToToolName = function (command) {
        ...
        if (command.startsWith('copilot_')) {
            return "pp_".concat(command);
        }
        var mappedName = commandMap[command];
        if (!mappedName) {
            throw new Error("Unknown command: ".concat(command));
        }
        return mappedName;
    };
    return VSCodeHierarchicalTools;
}());
exports.VSCodeHierarchicalTools = VSCodeHierarchicalTools;
