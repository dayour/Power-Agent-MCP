...
export class TemplateManager {
   ...
   */
  public getTemplate(scenarioId: string): TaskTemplate | undefined {
  ...
  public applyTemplate(
    ...
    customizations?: Partial<TaskConfiguration>
  ): TaskConfiguration {
    return {
      ...template.parameters,
      ...customizations
   ...
   */
  public validateTemplate(template: TaskTemplate): TemplateValidationResult {
    const errors: string[] = [];
   ...
   */
  public registerTemplate(template: TaskTemplate): boolean {
   ...
   */
  public listTemplates(): TaskTemplate[] {
   ...
   */
  public findTemplatesByTag(tag: string): TaskTemplate[] {
  ...
  public getTemplateSuggestions(context: {
    ...
    if (context.hasMultipleSolutions) {
      const multiSolutionTemplates = this.findTemplatesByTag('multi-solution');
      suggestions.push(...multiSolutionTemplates);
    }

    if (context.isProduction) {
      const prodTemplates = this.findTemplatesByTag('production');
      suggestions.push(...prodTemplates);
    }

    if (context.includesTesting) {
      const testTemplates = this.findTemplatesByTag('testing');
      suggestions.push(...testTemplates);
    }

    // Default suggestions if no specific context
    if (suggestions.length === 0) {
      suggestions.push(...this.findTemplatesByTag('basic'));
    }
